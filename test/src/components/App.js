...
import {
   ...
   initialFighter,
   initialOpponent,
} from "../reducers/pokemonReducer.js";
...
function App() {
   ...
   const [fighterState, fighterReducer] = useReducer(
      pokemonReducer,
      initialFighter
   );
   const [opponentState, opponentReducer] = useReducer(
      pokemonReducer,
      initialOpponent
   );
   ...
   async function fetchDamageModifierList(type) {
      await axios
         ...
         .then((res) => {
            return setDamageModifiers((damageModifiers) => [
               ...damageModifiers,
               {
                  id: res.data.id,
   ...
   async function fetchAllPokemonOfType() {
      await axios
         .get("https://pokeapi.co/api/v2/type/" + formState.type)
         .then((res) => {
      ...
      await axios
         ...
         .then((res) => {
            if (res.data.sprites.other.dream_world.front_default) {
               ...
               fighterReducer({
                  type: "species",
                  payload: res.data.species.name,
               });
               fighterReducer({
                  type: "hp",
                  payload: res.data.stats[0].base_stat,
               });
               fighterReducer({
                  type: "attack",
                  payload: res.data.stats[1].base_stat,
               });
               fighterReducer({
                  type: "defense",
                  payload: res.data.stats[2].base_stat,
               });
               fighterReducer({
                  type: "primarytype",
                  payload: res.data.types[0].type.name,
               });
               ...
               fighterReducer({
                  ...
                  payload: (
                     <img
                        src={res.data.sprites.other.dream_world.front_default}
                        alt="Your Pokemon"
                     />
                  ),
               });
               if (res.data.types[1]) {
                  fighterReducer({
                     type: "secondarytype",
                     payload: res.data.types[1].type.name,
                  });
               } else {
   ...
   async function fetchRandomPokemon() {
      await axios
         .get("https://pokeapi.co/api/v2/pokemon/" + randomInteger(0, 600))
         .then((res) => {
            opponentReducer({
               type: "species",
               payload: res.data.species.name,
      ...
      <LanguageContext.Provider value={locale}>
         <StyledWrapper>
            <StyledHeader className="Header">
               <h1 data-testid="language">
                  {translations[locale]["pokemon-battle"]}
               </h1>
               <div>
                  <img
                     src={eng}
